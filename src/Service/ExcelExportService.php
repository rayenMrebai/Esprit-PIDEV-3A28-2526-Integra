...
class ExcelExportService
{
    ...
    public function export(array $filters): string
    {
        ...
        // ✅ Correction erreur :51 — ob_get_clean() retourne string|false
    ...
    private function getSalairesWithFilters(array $filters): array
    {
        ...
        return array_values(array_filter($all, function (Salaire $s) use ($filters): bool {
            $date        = $s->getDatePaiement();
            $periodeType = (string) ($filters['periode_type'] ?? 'TOUS');

            if ($periodeType === 'MOIS' && $date !== null) {
            ...
            if ($periodeType === 'ANNEE' && $date !== null) {
            ...
            if ($periodeType === 'PERSONNALISEE' && $date !== null) {
                $debut = !empty($filters['date_debut'])
                    ? new \DateTime((string) $filters['date_debut'])
                    : null;
                $fin   = !empty($filters['date_fin'])
                    ? new \DateTime((string) $filters['date_fin'])
                    : null;
                if ($debut !== null && $date < $debut) {
                    return false;
                }
            ...
            if (!empty($statusFilters) && !in_array($s->getStatus(), $statusFilters, true)) {
                return false;
            }

            if (!empty($filters['montant_min']) && (float) $filters['montant_min'] > 0.0) {
                if ((float) $s->getTotalAmount() < (float) $filters['montant_min']) {
                    return false;
                }
            }
    ...
    /**
     * ✅ Types explicites sur $sheet et $value
     * @param int|float|string $value
     */
    private function write(Worksheet $sheet, int $col, int $row, int|float|string $value): void
    {
    ...
    /**
     * @param array<int, Salaire>   $salaires
     * @param array<string, mixed>  $filters
     */
    private function buildSalairesSheet(Worksheet $sheet, array $salaires, array $filters): void
    {
        ...
        $sheet->setCellValue(
            'A1',
            'LISTE DES SALAIRES - INTEGRA RH — ' . $this->getPeriodeLabel($filters)
        );
        ...
        foreach ($salaires as $salaire) {
            ...
            $this->write($sheet, 1, $row, (int) $salaire->getId());
            $this->write($sheet, 2, $row, $user !== null ? (string) $user->getUsername() : '');
            $this->write($sheet, 3, $row, $user !== null ? (string) $user->getEmail()    : '');
            $this->write($sheet, 4, $row, (float) $salaire->getBaseAmount());
            $this->write($sheet, 5, $row, $salaire->getBonusAmount());
            $this->write($sheet, 6, $row, (float) $salaire->getTotalAmount());
            $this->write($sheet, 7, $row, $salaire->getStatus());
            $this->write(
                $sheet, 8, $row,
                $salaire->getDatePaiement() !== null
                    ? $salaire->getDatePaiement()->format('d/m/Y')
                    : '—'
            );
     ...
     * @param array<string, mixed> $filters
     */
    private function buildStatistiquesSheet(Worksheet $sheet, array $salaires, array $filters): void
    {
        ...
        $payes   = count(array_filter($salaires, fn(Salaire $s) => $s->getStatus() === 'PAYÉ'));
        $enCours = count(array_filter($salaires, fn(Salaire $s) => $s->getStatus() === 'EN_COURS'));
        $crees   = count(array_filter($salaires, fn(Salaire $s) => $s->getStatus() === 'CREÉ'));

        /** @var array<int, float> $amounts */
        $amounts = array_map(fn(Salaire $s) => (float) $s->getTotalAmount(), $salaires);
        /** @var array<int, float> $bonuses */
        $bonuses = array_map(fn(Salaire $s) => $s->getBonusAmount(), $salaires);

        $totalMontant   = array_sum($amounts);
        $moyenneMontant = $total > 0 ? $totalMontant / $total : 0.0;

        // ✅ Correction erreurs :287 et :288 — max/min sur tableau potentiellement vide
        $maxMontant = !empty($amounts) ? max($amounts) : 0.0;
        $minMontant = !empty($amounts) ? min($amounts) : 0.0;

        $totalBonus   = array_sum($bonuses);
        $moyenneBonus = $total > 0 ? $totalBonus / $total : 0.0;
        ...
        foreach ($stats as [$label, $value, $isAmount]) {
            ...
            $sheet->setCellValue(
                "B{$row}",
                $isAmount
                    ? number_format((float) $value, 2, ',', ' ') . ' DT'
                    : $value
            );
     ...
     * @param array<string, mixed> $filters
     */
    private function buildBonusSheet(Worksheet $sheet, array $salaires, array $filters): void
    {
        $formatage = !empty($filters['appliquer_formatage']);
        ...
        foreach ($salaires as $salaire) {
            ...
            foreach ($salaire->getBonusRules() as $rule) {
                ...
                $this->write($sheet, 1, $row, $user !== null ? (string) $user->getUsername() : '');
                $this->write($sheet, 2, $row, (string) $rule->getNomRegle());
                $this->write($sheet, 3, $row, $rule->getPercentage() . '%');
                $this->write($sheet, 4, $row, $rule->getBonus());
                $this->write($sheet, 5, $row, (string) $rule->getConditionText());
    ...
    private function getPeriodeLabel(array $filters): string
    {
        ...
        return match ((string) ($filters['periode_type'] ?? 'TOUS')) {
            'MOIS'          => ($moisNoms[(int) ($filters['mois'] ?? 1)] ?? '')
                               . ' ' . (string) ($filters['annee_mois'] ?? date('Y')),
            'ANNEE'         => 'Année ' . (string) ($filters['annee'] ?? date('Y')),
            'PERSONNALISEE' => 'Du ' . (string) ($filters['date_debut'] ?? '—')
                               . ' au ' . (string) ($filters['date_fin'] ?? '—'),
    ...
    public function getFilename(array $filters): string
    {
        ...
        $suffix = match ((string) ($filters['periode_type'] ?? 'TOUS')) {
            'MOIS'          => ($moisNoms[(int) ($filters['mois'] ?? 1)] ?? 'mois')
                               . '_' . (string) ($filters['annee_mois'] ?? date('Y')),
            'ANNEE'         => (string) ($filters['annee'] ?? date('Y')),
            'PERSONNALISEE' => (string) ($filters['date_debut'] ?? '')
                               . '_au_' . (string) ($filters['date_fin'] ?? ''),
